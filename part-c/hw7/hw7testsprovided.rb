...
s = Shift.new(-3.0,3.0, Let.new("x", LineSegment.new(1.7,2.2,3.7,4.2), Shift.new(0.3,-0.2, Var.new("x"))))
s1 = s.preprocess_prog.eval_prog([])
if not (s1.x1 == -1.0 and s1.y1 == 5.0 and s1.x2 == 1.0 and s1.y2 == 7.0)
	puts "Shift should evaluate its arguments and then shift"
end
...
s = Shift.new(-3.0,3.0,Shift.new(0.3,-0.2,LineSegment.new(1.7,2.2,3.7,4.2)))
s1 = s.preprocess_prog.eval_prog([])
if not (s1.x1 == -1.0 and s1.y1 == 5.0 and s1.x2 == 1.0 and s1.y2 == 7.0)
	puts "Shift should evaluate its arguments and then shift"
end
